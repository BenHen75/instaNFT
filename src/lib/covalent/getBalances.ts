...
export async function getBalances(
  ...
  { balances: Array<any>; success: true } | { error: string; success: false }
> {
  if (!accountAddress) return { error: "No account address", success: false };

  const url = `https://api.covalenthq.com/v1/${chain_id}/address/${accountAddress}/balances_v2/?nft=false&key=`+process.env.REACT_APP_COVALENT_KEY;

  try {
    const res = await axios.get(url);

    if (res.status !== 200) {
      return { error: "No balances found", success: false };
    }

    console.log(res)
    
    const balances = transformArray(res.data.data.items);
    ...
    errorHandler(error, "getBalances error");
    return {
      error: error.message,
      success: false,
    };
